...
const SeatSelectionPage = () => {
  ...
  const toggleSeat = (seat) => {
    ...
    setSelectedSeats(prev =>
      prev.find(s => s.seatNo === seat.seatNo)
  ...
  if (loading) return <><Navbar /><div className="loading-page"><div className="spinner"/><p>Loading show...</p></div></>;
  if (!show) return <><Navbar /><div className="empty-state"><p>Show not found</p></div></>;
                  ...
                  return (
                    <div key={row} className="seat-row">
                      ...
                      {rowSeats.map(seat => {
                        ...
                        return (
                          <div key={seat.seatNo} className={cls} title={`${seat.seatNo} - ${seat.type} - ₹${getSeatPrice(seat)}`}
              ...
              <div className="seat-legend">
                {[
                  ...
                  <div key={l.label} className="legend-item">
                    <div className={l.cls} style={{ width: 18, height: 16, borderRadius: 3, cursor: 'default' }} />
                    <span>{l.label}</span>
                  </div>
                ))}
              </div>
          ...
          <div style={{ maxWidth: 520, margin: '0 auto' }}>
            <div className="card" style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: '0.9rem' }}>
                ...
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: 10, display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: '1.05rem' }}>
                  <span>Total</span>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                <input className="form-input" placeholder="Coupon code (e.g. WELCOME100)"
                  value={couponCode} onChange={e => setCouponCode(e.target.value.toUpperCase())} />
                <button className="btn btn-secondary" onClick={applyCoupon} disabled={couponLoading}>
                  {couponLoading ? '⏳' : 'Apply'}
                </button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-primary btn-lg" onClick={handleCreateBooking} disabled={bookingLoading}>
                {bookingLoading ? '⏳ Creating Booking...' : '🎟️ Confirm & Pay'}
              </button>
            </div>
          </div>
        )}
